...
NSString *const BWRouterParameterURL = @"BWRouterParameterURL";
NSString *const BWRouterParameterCompletion = @"BWRouterParameterCompletion";
NSString *const BWRouterParameterUserInfo = @"BWRouterParameterUserInfo";
...
@end
@implementation BWRouter

+ (instancetype)sharedInstance
{
	static BWRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(BWRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
	...
	if (parameters) {
		BWRouterHandler handler = parameters[@"block"];
		if (completion) {
			parameters[BWRouterParameterCompletion] = completion;
		}
		if (userInfo) {
			parameters[BWRouterParameterUserInfo] = userInfo;
		}
		if (handler) {
...
{
	BWRouter *router = [BWRouter sharedInstance];

	URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
	NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
	BWRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(BWRouterObjectHandler)handler
{
	[[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(BWRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(BWRouterObjectHandler)handler
{
	NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
	if (handler && subRoutes) {
		subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
	...
	parameters[BWRouterParameterURL] = url;
